...
public class JobService {
    ...
    public JobService(JobRepository jobRepository, SoftSkillRepository softSkillRepository,
            ...
            BenefitsRepository benefitsPerksRepository, PerksRepository perksRepository,
            DepartmentRepository departmentRepository, JobTypeRepository jobTypeRepository,
    ...
    public Job save(Job job) {
        ...
        List<String> PerksString = new ArrayList<String>();
        job.getPerks().forEach(per -> PerksString.add(per.getPerksName()));
        
        job.setPerks(null);
        
        Iterable<Perks> allPerks = perksRepository
                .findAllByPerksNameInAndActiveIsTrue(PerksString);

        allPerks.forEach(per -> per.addJob(job));
        job.setPerks(new HashSet<>(perksRepository.saveAll(allPerks)));
        ...
        Department department = departmentRepository.findByDepartmentName(job.getDepartments().getDepartmentName());
        job.setDepartments(department);      
        
        Location location = locationRepository.findByLocationName(job.getLocations().getLocationName());
